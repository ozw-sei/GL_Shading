...
CmyPet::CmyPet()
{
  ...
  //胴体
	trunk = CVector( 0.4, 0.36, 0.6);
  trunkBefore = CVector(0.44, 0.44, 0.44);
	trunkAfter = CVector(0.44, 0.44, 0.44);
	//頭部
	head = CVector( 0.46, 0.4, 0.46);
  //目
  eye = CVector( 0.08, 0.08, 0.08);
	//鼻
	nose = CVector( 0.06, 0.06, 0.06);
  //耳
  ear = CVector( 0.12, 0.3, 0.12);
	//口
	mouth = CVector(0.28, 0.28, 0.28);
	//上脚（前脚，後脚)
	leg1 = CVector(0.14, 0.12, 0.14);
	//下脚（前脚，後脚)
	leg2 = CVector(0.13, 0.12, 0.13);
  //尾
  tail = CVector( 0.2, 0.2, 0.26);
  //脚を取り付ける位置
	legPos.x = trunk.x / 2.0 - 0.06;
  legPos.y = trunk.y / 2.0 - 0.06;
  legPos.z = trunk.z / 2.0 - 0.08;
  //脚全長
